 ...
 * ```
 */
export function parseSgxQuote(report) {
    if (report.length < SGX_MIN_QUOTE_SIZE) {
        throw new Error(`Invalid SGX quote: ${report.length} bytes, minimum ${SGX_MIN_QUOTE_SIZE} required`);
    }
    ...
    if (quoteVersion !== SGX_DCAP_V3_VERSION) {
        throw new Error(`Unsupported SGX quote version: ${quoteVersion} (expected ${SGX_DCAP_V3_VERSION} for DCAP v3)`);
    }
 ...
 * ```
 */
export async function verifyAttestation(report, config) {
    if (report.length === 0) {
