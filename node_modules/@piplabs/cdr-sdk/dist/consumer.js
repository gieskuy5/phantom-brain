...
import { decryptPartial as eciesDecrypt, tdh2Combine, decryptFile, generateEphemeralKeyPair, } from "@piplabs/cdr-crypto";
import { PartialCollectionTimeoutError, InvalidParamsError, CidIntegrityError, EmptyVaultError, } from "./errors.js";
...
export class Consumer {
    ...
    async collectPartials(params) {
        const { uuid, requesterPubKey, timeoutMs = 60_000, pollIntervalMs = 3_000, onInvalidPartial, attestationConfig, pinnedCiphertext, } = params;
        if (!requesterPubKey) {
                    ...
                    for (const sub of group.submissions) {
                        ...
                        if (trustSet && !trustSet.has(sub.validator)) {
                            ...
                            if (!reported.has(dedupeKey)) {
                                ...
                                onInvalidPartial?.(event, new Error(`attestation rejected for validator ${sub.validator}`));
                            }
                            continue;
                        }
                        accepted.push(event);
     ...
     */
    async getTrustSet(round, config, cache) {
    ...
    async accessCDR(params) {
        if ((params.requesterPubKey && !params.recipientPrivKey) ||
        ...
        const globalPubKey = params.globalPubKey ?? (await this.observer.getGlobalPubKey());
        try {
...
// Helpers
// ---------------------------------------------------------------------------
function submissionToEvent(sub, uuid) {
    return {
