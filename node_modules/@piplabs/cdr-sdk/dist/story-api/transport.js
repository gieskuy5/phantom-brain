...
export async function fetchJSON(url, opts) {
    ...
    for (let attempt = 0; attempt < RETRY_ATTEMPTS; attempt++) {
        ...
        try {
            ...
            // 5xx → retry
            if (res.status >= 500) {
                lastError = new StoryApiError(res.status, `5xx response: ${bodyText.slice(0, 200)}`);
            ...
            if (res.status === 404) {
                throw new StoryApiNotFoundError(envelope?.error ?? bodyText.trim() ?? "not found");
            }
            if (envelope == null) {
                throw new StoryApiError(res.status, `non-JSON body: ${bodyText.slice(0, 200)}`);
            }
            if (envelope.code !== 200 || envelope.msg == null) {
                throw new StoryApiError(res.status, envelope.error || `code=${envelope.code}`, envelope.code);
            }
            return envelope.msg;
        }
        catch (err) {
            // Pass through abort + typed errors (no retry)
            if (err instanceof StoryApiNotFoundError)
                throw err;
            if (err instanceof StoryApiError && err.status < 500)
                throw err;
            if (opts.signal?.aborted)
                throw err;
